...
describe Segment do
  ...
  context 'when parameters is wrong' do
    it 'raises exception Figure::ValidateError' do
      expect { described_class.new(x: 'wrong', y: 'wrong', x1: 1, y1: 1)
      }.to raise_error(Figure::ValidateError, 'Parameters should be numeric for Segment')

      expect { described_class.new(x: 1, y: 'wrong', x1: 2, y1: 1)
      }.to raise_error(Figure::ValidateError, 'Parameters should be numeric for Segment')
  ...
  describe '#==' do
    ...
    it 'returns false when segment are not equal' do
      expect(described_class.new(x: 2, y: 3, x1: 21, y1: 6))
        .to_not eq(described_class.new(x: 2, y: 3, x1: 5, y1: 6))
    end
  end
  ...
  describe 'coords' do
    ...
    it do
      expect(segment.first_point).to eq(Point.new(x: 2, y: 3))
      expect(segment.second_point).to eq(Point.new(x: 21, y: 6))
    end
  end
  ...
  describe '#include_point?' do
    ...
    it 'returns true when Point is in segment' do
      expect(segment.include_point?(Point.new(x: 2, y: 3))).to be_truthy
    end

    it 'returns false when Point isn\'t in segment' do
      expect(segment.include_point?(Point.new(x: 2, y: -5))).to be_falsey
    end
  end
  ...
  describe '#to_straight_line' do
    ...
    it 'returns exception Segment::ConvertError when segment is vertical line' do
      expect { described_class.new(x: 2, y: 3, x1: 2, y1: 6).to_straight_line}
        .to raise_error(Segment::ConvertError, 'Can not to convert to straight line')
    end
